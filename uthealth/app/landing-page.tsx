...
const LandingPage: React.FC = () => {
    ...
    const [dates, setDates] = useState<DateItem[]>(
        generateSurroundingDates(new Date())
    )
    ...
    const fetchPrescribedExercises = async (date: Date) => {
        ...
        try {
            ...
            const { data, error } = await supabase
                .from('prescription')
                .select(
                    'prescription_id, exercise_id, start_date, end_date, days'
                )
            ...
            if (data && data.length > 0) {
                ...
                const exerciseIds = filteredPrescriptions.map(
                ...
                if (exerciseData) {
                    const newActivities: Activity[] = exerciseData.map(
                        (exercise) => ({
            ...
            <View className="flex-1">
                ...
                <ScrollView className="flex-1">
                    ...
                    <View className="mt-4">
                        <FlatList
                            ...
                            onScrollToIndexFailed={(info) => {
                                // fallback if the list still isn’t ready
                    ...
                    <View className="px-4 mt-6 mb-20">
                        ...
                        <TouchableOpacity className="flex-row items-center justify-between bg-white rounded-lg py-4 px-4 mt-2 shadow-sm">
                            <Text
